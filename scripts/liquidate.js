...
exports.handler = async function (credentials) {
    ...
    const vaultController = new ethers.Contract(VAULTCONTROLLER_ADDRESS, VAULTCONTROLLER_ABI, signer);

    // We check that contract is not paused to be sure that we can liquidte
    const isPaused = await vaultController.paused();
    if (!isPaused) {
        // log saying that the vault is not paused
        console.log("Vault is not paused");
        // We get the total number of vault that exist
        const totalVault = await vaultController.vaultsMinted();
        console.log('Total vault :', totalVault.toString());
        // iterate between all existing vault to check if they can be liquidated
        for (let vaultId = 1; vaultId <= totalVault; vaultId++) {
            var peekCheckVault = await vaultController.peekCheckVault(vaultId);
            ...
            if (!peekCheckVault) {
                ...
                for(tokenAddr in vaultSummary.tokenAddresses){
                    ...
                    console.log("Amount to liquidate : ", amountToLiquidate.toString());
                    // liquidate vault if they can be liquidated
                    if(parseInt(amountToLiquidate) > 0) {
                        const liquidated = await vaultController.liquidateVault(vaultId, tokenAddr, amountToLiquidate);
                        console.log(`Amount liquidated: ${liquidated}`);
                    }
                }
            } else {
                // log saying that the vault (with vault id) cannot be liquidated
                console.log(`Vault #${vaultId} cannot be liquidated`);
            }
        }
    } else {
